(function() {
  ...
  function runCodeGrammar(grammar, tags, tokens, tokenTagIndices, codeStack) {
    // From debugging a bit, it seems the function is invoked per line
    // but I haven't yet figured out how it's determined which lines are used.
    // The tokens represent some bit of grammar in the file content as objects
    // like: {scopes: ['markup.added.diff', 'unimportant'], value: 'foo'}.

    // Given a list of tokens in a line, combine all the content that is code (see isNotCode() towards bottom of the file)
    // into a string (removedLine/addedLine based on whether its a line that was added or removed).
    var removedLine = ''
    var addedLine = ''
    ...
    var replacementTags = []
    var newIdx = {} // start collecting indices of tags
    if (removedIdx) {
    ...
    // From here on, it looks like an attempt to reconstruct the tag tree.
    // I'm not sure how to explain what the tag tree is because I don't yet understand it.
    // It seems like a numerical representation of the grammar. 
    // This comment(https://github.com/atom/atom/issues/10350#issuecomment-248618306)
    // references the tags and visualizes the concept though it doesn't have an explanation.
    for (let i = tokens.length - 1; i >= 0; i--) {
      ...
      var currentIdx = 0
      var newTags
      removedIdx = _.isUndefined(newIdx.removed) ? removedIdx : newIdx.removed //Redundant?
      addedIdx = _.isUndefined(newIdx.added) ? addedIdx : newIdx.added //Redundant?
